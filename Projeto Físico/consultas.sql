...
SELECT NOME_NPC
FROM MATA
WHERE NOME_PROTAGONISTA LIKE 'Toni%';

-- PROJETA TODOS OS CARROS QUE O PROTAGONISTA TROCOU DE ESTACAO DE RADIO--
SELECT VEICULO.*
FROM VEICULO
...
-- PROJETA TODOS OS PROTAGONISTAS QUE AJUDARAM O PROTAGONISTA 'Toni' --
SELECT PROTAGONISTA.*
FROM PROTAGONISTA
WHERE EXISTS (
	SELECT 1
	FROM AJUDA
	WHERE AJUDA.AJUDADO LIKE 'Toni' 
	   AND PROTAGONISTA.NOME = AJUDA.AJUDANTE
)

-- PROJETA AS MÚSICAS QUE TOCARAM EM TODAS AS MISSÕES --
SELECT DISTINCT MUSICA.*
FROM MUSICA
WHERE MUSICA.NOME IN (
	SELECT DISTINCT TOCA.NOME_MUSICA
	FROM TOCA, DIRIGE, MISSAO
	WHERE DIRIGE.DESCRICAO = MISSAO.DESCRICAO 
		AND DIRIGE.NOME_VEICULO = TOCA.NOME_VEICULO
	GROUP BY TOCA.NOME_MUSICA
	HAVING COUNT(DISTINCT DIRIGE.DESCRICAO) = (
			SELECT COUNT(*)
			FROM MISSAO
		)
	)
...
-- PROJETA TODAS AS MISSÕES QUE O PROTAGONISTA FEZ SOLO --
SELECT MISSAO.*
FROM MISSAO
WHERE NOT EXISTS (
	SELECT 1
	FROM AJUDA
	WHERE MISSAO.NOME_PROTAGONISTA = AJUDA.AJUDADO 
		AND AJUDA.AJUDANTE IS NOT NULL
)
